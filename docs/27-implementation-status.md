...
## 37. Consumption log: date filtering + printing; add-item-with-stock; opening cost (2026-09-21)

### 37.1 Consumption log (`/consumption`) - filters, report, print
- **Source/time semantics (decision):** the log is filtered on the RECORDING instant (`consumption_records.created_at`), not the user-typed `consumption_date`, because a rolling "last 24 hours" is only meaningful for an instant. Covered by the existing `ix_consumption_records_tenant_keyset` (company_id, created_at, id) - no new index or migration.
- **API:** `GET /api/v1/consumption?from&to&restaurantId&limit&cursor` (extended, not duplicated). No `from`/`to` = rolling last 24 hours ending now (server-side, never "midnight-to-now"). `from`/`to` = calendar dates in the COMPANY timezone (`company_settings.timezone_id`, default Africa/Cairo), inclusive of both; `from = to` is a single date. Day boundaries are `[00:00 of from, 00:00 of to+1)` converted to UTC (a DST change that swallows midnight - Cairo - starts the day at 01:00). Validation: both or neither, `to >= from`, span <= 366 days, else `400 INVALID_DATE_RANGE`; a malformed date is `400 INVALID_REQUEST`. Keyset pagination is preserved. Rows now carry server-resolved names (`itemName`, `unitName`, `restaurantName`, `recordedByName`) and `recordedAtLocal` (company-timezone `yyyy-MM-dd HH:mm`). No cost/price field exists in any response.
- **Report:** `GET /api/v1/consumption/report` - same filter, same scope. Returns the period actually applied (`kind`, `from`, `to`, `fromUtc`, `toUtc`, `timezoneId`), `generatedAtLocal`, company name, restaurant context, up to 5,000 rows (`truncated` flagged) and per-item/unit totals plus the record count computed IN THE DATABASE over the whole filtered set.
- **Authorization:** unchanged - `consumption:view`; tenant query filter; a Restaurant Supervisor is confined to their own restaurant(s) (a foreign `restaurantId` is 403, several restaurants need an explicit one). The scope/validation logic lives in one helper shared by list and report.
- **Bug found by the new tests and fixed:** `company_settings` has no tenant query filter, so reading "the" settings row returned another company's timezone; the timezone lookup now filters by the caller's company id.
- **UI:** filter bar (آخر 24 ساعة | تاريخ محدد | فترة زمنية, from/to date pickers, apply, restaurant filter when several), the active period is always shown ("الفترة المعروضة"), Arabic empty state per period, and "طباعة التقرير" which fetches the report from the server for exactly the applied filter and prints ONLY a `#consumption-print-root` portal (`@media print` hides everything else): title "سجل الاستهلاك", period wording (`الفترة: آخر 24 ساعة` / `التاريخ: 10/09/2026` / `الفترة: من 01/09/2026 إلى 10/09/2026`), timezone, generated date-time, company/restaurant, records, totals.
- **Tests:** `ConsumptionDateFilterTests` (18: default 24h, single date, inclusive range, exact boundaries, timezone (Cairo/Tokyo), DST-midnight day, empty period, pagination in a window, company isolation, supervisor scope, permission 403, no financial fields, report == filter, invalid ranges); browser `e2e/consumption-filters-and-print.spec.ts` (default 24h -> single date -> empty -> range -> print period wording/records/totals, sidebar+filters absent in print media).

### 37.2 Warehouse stock screen
- **Add item with stock:** "إضافة صنف بالرصيد" (holders of `stock:direct_set` + `items:create`) creates the item (name, category, base unit, with quick-add for category/unit) and sets its opening balance in this warehouse in one dialog. If the item is created but the balance call fails the user is told and can set it with the row's pencil.
- **Opening cost:** `PUT /warehouses/{id}/stock/{itemId}` accepts an optional `unitCost` (>= 0). For an INCREASE it posts an `OpeningBalance` ledger movement carrying that cost (the weighted-average cost is recomputed); without a cost it stays a plain `PhysicalAdjustment`. Supplying a cost requires cost visibility (`IFinancialProjection.IsVisible`, i.e. Owner today) - otherwise `403`, and the UI only shows the "تكلفة الوحدة" field to such accounts. Tests: `DirectStockSetTests` (13). Browser: `e2e/stock-add-item.spec.ts`.
